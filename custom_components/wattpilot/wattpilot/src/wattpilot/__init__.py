...
class Wattpilot:
    ...
    def __on_auth(self, wsapp, message):
        ...
        hash1 = hashlib.sha256(
            message.token1.encode() + self._hashedpassword
        ).hexdigest()
        hash = hashlib.sha256(
            (self._token3 + message.token2 + hash1).encode()
        ).hexdigest()
        response = {}
        response["type"] = "auth"
    ...
    def __update_hashedpassword(self, password=None, serial=None):
        ...
        if (password is None) or (serial is None):
            _LOGGER.info(
                "__update_hashedpassword: password or serial empty - keep current _hashedpassword"
            )
            return
        _LOGGER.debug(
            "__update_hashedpassword: generating password hash of type: %s",
            self._authhashtype,
        )
        if self._authhashtype == CONST_HASH_PBKDF2:
            self._hashedpassword = base64.b64encode(
                hashlib.pbkdf2_hmac(
                    "sha512", password.encode(), self.serial.encode(), 100000, 256
                )
            )[:32]
            ...
            _LOGGER.error(
                "__update_hashedpassword: unknown authhashtype: %s", self._authhashtype
            )
    ...
    def __bcryptjs_encodeBase64(self, s: str, length: int) -> str:
        ...
        if s.isdigit():  # numeric only serial
            ...
            raise ValueError("Check serial string - should be digits only: %s", s)
    ...
    def __send(self, message, secure=False):
        ...
        if secure:
            ...
            h = hmac.new(
                bytearray(self._hashedpassword),
    ...
    def __on_response(self, message):
        if message.success:
            ...
            _LOGGER.error(
                "Error Sending Request %s. Message: %s",
                message.requestId,
                message.message,
            )
    ...
    def __on_message(self, wsapp, message):
        ...
        if (
            msg.type == "response"
        ):  # Response Message -> Received after sending a update and contains result of update
            self.__on_response(msg)
        if (
            msg.type == "authSuccess"
        ):  # Auth Success -> Received after sending  correct authentication message
            self.__on_AuthSuccess(msg)
        if (
            msg.type == "authError"
        ):  # AUth errot -> Received after sending incorrect authentication message (e.g. wrong password)
            self.__on_AuthError(msg)
        if (
            msg.type == "fullStatus"
        ):  # Full Status -> Received after successfull connection. Contains all Properties of Wattpilot
            self.__on_FullStatus(msg)
        if (
            msg.type == "deltaStatus"
        ):  # Delta Status -> Whenever a property changes a Delta Status is send
            self.__on_DeltaStatus(msg)
        if msg.type == "clearInverters":  # Unknown
            self.__on_clearInverters(msg)
        if (
            msg.type == "updateInverter"
        ):  # Contains information of connected Photovoltaik inverter / powermeter
