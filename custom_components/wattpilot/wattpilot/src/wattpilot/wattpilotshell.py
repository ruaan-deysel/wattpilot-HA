...
def wp_get_child_prop_value(cp):
    ...
    if ppd["jsonType"] == "array":
        ...
        if parent_value == None:
            ...
            isinstance(parent_value, SimpleNamespace)
            and cpd["valueRef"] in parent_value.__dict__
        ):
...
def wp_get_all_props(available_only=True):
    ...
    if available_only:
        ...
        props = {
            k: (wp.allProps[k] if k in wp.allProps else None)
...
class WattpilotShell(cmd.Cmd):
    ...
    def _complete_message(self, text, sender=None):
        ...
        return [
            md["key"]
            for md in wpdef["messages"].values()
            if (not sender or md["sender"] == sender) and md["key"].startswith(text)
        ]
    ...
    def _complete_propname(self, text, rw=False, available_only=True):
        ...
        return [
            k
            for k in wp_get_all_props(available_only).keys()
            if (
                not rw
                or (
                    "rw" in wpdef["properties"][k]
                    and wpdef["properties"][k]["rw"] == "R/W"
                )
            )
    ...
    def _complete_values(self, text, line):
        ...
        if len(token) == 3 and text in wpdef["properties"]:
    ...
    def complete_ha(self, text, line, begidx, endidx):
        ...
        if len(token) == 3 and token[1] == "discover":
            return self._complete_list(
                [
                    p
                    for p in self._complete_propname(text, available_only=True)
                    if p not in HA_PROPERTIES
                ],
                text,
            )
        if len(token) == 3 and token[1] in ["enable", "disable", "undiscover"]:
    ...
    def complete_mqtt(self, text, line, begidx, endidx):
        ...
        if len(token) == 2:
            return self._complete_list(
                ["properties", "publish", "start", "status", "stop", "unpublish"], text
            )
        if len(token) == 3 and token[1] == "publish":
            return self._complete_list(
                [
                    p
                    for p in self._complete_propname(text, available_only=True)
                    if p not in MQTT_PROPERTIES
                ],
                text,
            )
        if len(token) == 3 and token[1] == "unpublish":
    ...
    def complete_properties(self, text, line, begidx, endidx):
        return self._complete_propname(text, rw=False, available_only=False) + [
            "<propRegex>"
        ]
    ...
    def complete_set(self, text, line, begidx, endidx):
        ...
        if len(token) == 3 and token[1] in wpdef["properties"]:
    ...
    def complete_unwatch(self, text, line, begidx, endidx):
        ...
        if len(token) == 3 and token[1] == "message":
            return self._complete_list(self.watching_messages, text)
        if len(token) == 3 and token[1] == "property":
    ...
    def _print_prop_info(self, pd, value):
        ...
        if pd["key"] in wp.allProps.keys():
            ...
            print(
                "  NOTE: This property is currently not provided by the connected device!"
            )
    ...
    def _get_props_matching_regex(self, arg, available_only=True):
        ...
        props = {
            k: v
            for k, v in wp_get_all_props(available_only).items()
        ...
        props = {
            ...
            if re.match(
                r"^" + value_regex + "$",
                str(mqtt_get_encoded_property(wpdef["properties"][k], v)),
                flags=re.IGNORECASE,
            )
        }
        return props
...
def mqtt_get_remapped_value(pd, mapped_value):
    ...
    if "valueMap" in pd:
        if mapped_value in pd["valueMap"].values():
            remapped_value = json.loads(
                str(
                    list(pd["valueMap"].keys())[
...
def mqtt_publish_property(wp, mqtt_client, pd, value, force_publish=False):
    ...
    if not (force_publish or MQTT_PROPERTIES == [""] or prop_name in MQTT_PROPERTIES):
        ...
        return
    property_topic = mqtt_subst_topic(
        MQTT_TOPIC_PROPERTY_STATE,
        {
...
def mqtt_publish_message(wp, wsapp, msg, msg_json):
    ...
    if MQTT_PUBLISH_MESSAGES == "true" and (
        MQTT_MESSAGES == [] or MQTT_MESSAGES == [""] or msg.type in MQTT_MESSAGES
    ):
        message_topic = mqtt_subst_topic(
            MQTT_TOPIC_MESSAGES,
            {
                "baseTopic": MQTT_TOPIC_BASE,
                "serialNumber": wp.serial,
...
def ha_discover_property(
    wp, mqtt_client, pd, disable_discovery=False, force_enablement=None
):
    ...
    _LOGGER.debug(f"Homeassistant config: haInfo={ha_info}, component={component}")
    title = pd.get("title", pd.get("alias", name))
    _LOGGER.debug(f"Publishing HA discovery config for property '{name}' ...")
    ...
    base_topic = mqtt_subst_topic(MQTT_TOPIC_PROPERTY_BASE, topic_subst_map, False)
    ha_discovery_config = ha_get_default_config_for_prop(pd) | {
        "~": base_topic,
        "name": title,
        "object_id": object_id,
        "unique_id": unique_id,
        "state_topic": mqtt_subst_topic(
            MQTT_TOPIC_PROPERTY_STATE, topic_subst_map, False
        ),
    ...
    ha_discovery_config = dict(
        list(ha_discovery_config.items()) + list(ha_config.items())
    )
    ...
    _LOGGER.debug(f"Publishing property '{name}' to {topic_cfg}: {payload}")
    ...
    if pd.get("rw", "") == "R/W" and component != "sensor":
        ...
        mqtt_client.publish(
            mqtt_subst_topic(
                HA_TOPIC_CONFIG, topic_subst_map | {"component": "sensor"}
            ),
            payload,
            retain=True,
        )
    if WATTPILOT_SPLIT_PROPERTIES and "childProps" in pd:
        for p in pd["childProps"]:
            ha_discover_property(
                wp, mqtt_client, p, disable_discovery, force_enablement
            )
...
def ha_get_discovery_properties():
    ...
    if ha_properties == [""] or ha_properties == []:
        ha_properties = [
            p["key"] for p in wpdef["properties"].values() if ha_is_default_prop(p)
        ]
...
def ha_publish_initial_properties(wp, mqtt_client):
    ...
    global wpdef
    _LOGGER.info(
        "Publishing all initial property values to MQTT to populate the entity values ..."
    )
